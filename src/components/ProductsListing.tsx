...
const ProductsListing = async () => {
  // External API se data fetch karein
  const response = await fetch('https://hackathon-apis.vercel.app/api/products');
  const products: CardItem[] = await response.json();

  // Unique categories nikalne ke liye
  const categories = Array.from(new Set(products.map((product) => product.categoryName))).map((name) => ({
    name,
    slug: name.toLowerCase().replace(/\s+/g, '-'), // Slug generate karna
  }));
